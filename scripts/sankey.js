...
d3.sankey = function () {
  var sankey = {},
    nodeWidth = 24,
    nodePadding = 18, // was 8, needs to be much bigger. 
    // these numbers are actually overwritten in the html when we instantiate the viz!
    size = [1, 1],
    nodes = [],
    links = [];
  ...
  sankey.link = function () {
    ...
    function link(d) {
      ...
      var x0 = d.source.x + d.sy + d.dy / 2,
        ...
        curvature = Math.abs(x0 - x1) > nodeWidth * 3 ? 1 : 0.5, //TODO: review this
        y2 = yi(curvature),
        y3 = yi(1 - curvature);
  ...
  function computeNodeLinks() {
    ...
    links.forEach(function (link) {
      ...
      // debugger
      if (typeof source !== "object") source = link.source = nodeById.get(link.source);
      if (typeof target !== "object") target = link.target = nodeById.get(link.target);
  ...
  function computeNodeBreadths(iterations) {
    ...
    var ky = (size[0] - (nodesByBreadth[0].length - 1) * sankey.nodePadding()) / d3.sum(nodesByBreadth[0], value);
    // I'd like them to be much bigger, this calc doesn't seem to fill the space!?
    var min = 1
    nodesByBreadth.forEach(function (nodes) {
      nodes.forEach(function (node, i) {
        node.x = i;
        node.dy = Math.max(min, node.value * ky - nodePadding);
        min = Math.min(min, Math.abs(node.value * ky - nodePadding))
        // node.dy = Math.abs(node.value * ky - nodePadding);
    ...
    function resolveCollisions() {
      nodesByBreadth.forEach(function (nodes) {
        ...
        for (i = 0; i < n; ++i) {
          ...
          dy = x0 - node.x;
          if (dy > 0) node.x += dy;
          x0 = node.x + node.dy + sankey.nodePadding();
        }

        // If the rightmost node goes outside the bounds, push it left.
        dy = x0 - sankey.nodePadding() - size[0]; // was size[1]
        if (dy > 0) {
          x0 = node.x -= dy;
          ...
          for (i = n - 2; i >= 0; --i) {
            node = nodes[i];
            dy = node.x + node.dy + sankey.nodePadding() - x0; // was y0
            if (dy > 0) node.x -= dy;
            x0 = node.x;
          }
        }
